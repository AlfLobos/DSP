...
if __name__ == '__main__':
    ...
    np.random.seed(1295)
    vector_r = vector_r * 0.5 * np.random.uniform(low = 0.5, high =1.5, size= len(vector_r))
    ...
    cL2List = [10, 10, 10, 10, 10, 10, 100, 100, 100, 100, 100, 100, \
        100, 100, 100]

    cL2IndList = [10, 10, 10, 10, 10, 10, 100, 100, 100, 100, 100, \
        100, 100, 100, 100]

    num_it = 50000

    grMult =  [np.round(0.2 + 0.02*i, decimals = 3) for i in range(15)]

    auxVectForTau = [-4.0 + 0.1*i for i in range(50,65)] 
    ...
    with open(suffix + 'TableCriteo05Emp_' + str(indexToUse+50) + '.csv', 'w') as csvfile:
