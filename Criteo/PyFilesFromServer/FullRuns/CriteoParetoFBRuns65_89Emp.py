...
if __name__ == '__main__':
    ...
    np.random.seed(1295)
    vector_r = vector_r * 1.0 * np.random.uniform(low = 0.5, high =1.5, size= len(vector_r))
    ...
    cL2List = [10, 10, 10, 10, 10, 10, 10, 10, 10, 10, \
        10, 10, 10, 10, 10, 10, 10, 1, 1, 1, 1, 1, 1, 1]
    ...
    grMult =  [np.round(0.2 + 0.02*i, decimals = 3) for i in range(65,89)]

    auxVectForTau = [-4.0 + 0.1*i for i in range(65,89)] 
    tauMult = list(np.round(np.exp(np.array(auxVectForTau)), decimals = 3))
    ...
    ## To all indexes I added a 65
    indexToUse = int(args.id) - 65
    ...
    with open(suffix + 'TableCriteoFBEmp_' + str(indexToUse+65) + '.csv', 'w') as csvfile:
